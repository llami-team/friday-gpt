...
* 그 외 파일도 필요하다면 추가해서 작성해주세요.
* 규격에 맞게 Node.js 프로젝트 개발 진행을 바로 시작해주세요.
* JSON 내 값은 모두 JSON.parse() 로 파싱 가능한 형태로 인코딩해서 출력해주세요.
* JSON 만을 반복없이 출력해주세요.
* JSON 뒤에 다른 내용을 반복하지마세요.
* 유저로부터 입력받은 코드를 그대로 복사해서 사용하지마세요.

* 응답 JSON 규격과 예시는 다음과 같습니다.
{
  "./package.json": "{...}",
